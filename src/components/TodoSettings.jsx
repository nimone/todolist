        ...
        {settings.isSignedIn ? (
          ...
          <Button onClick={syncGoogleTasks}>
            <img
              ...
              src="https://raw.githubusercontent.com/AliasIO/wappalyzer/master/src/drivers/webextension/images/icons/Google.svg"
            />
...
function ThemedButton({ children, theme = "", selected, ...props }) {
  return (
    <button
      className={`w-6 h-6 m-1 focus:outline-none rounded-full ${
        theme ? "bg-gradient-to-br" : ""
      } ${theme} ${selected ? "ring-2 ring-white/80" : ""}`}
      {...props}
    >
      {children}
    </button>
